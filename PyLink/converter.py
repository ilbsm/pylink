...
def print_namespace(args):  # format namespaces for debug option
    ...
    if not args.fourcolumn: print(
                color.BOLD + "fourcolumn\t" + color.END + color.RED + str(args.fourcolumn) + color.END)
...
def read_header(pdb_file, args):  # reading header of PDB file
    ...
    with open(input_file, 'r') as f:
        if type(debug) is list: print(time.strftime("%d %b %H:%M:%S",
                                                    time.gmtime()) + " File " + input_file + " opened. Analyzing the header.")
        for line in f:
...
def parse_atoms(args):  # models,chains,residues,atoms,debug):
    ...
    debug = args['debug']
    if type(debug) is list: print(time.strftime("%d %b %H:%M:%S",
                                                time.gmtime()) + " Parsing permitted atoms. The values supplied:\n" + color.BOLD + "Models:" + color.END + str(
        models) + color.BOLD + "\tChains:" + color.END + str(chains) + color.BOLD + "\tResidues:" + color.END + str(
        residues) + color.BOLD + "\tAtoms:" + color.END + str(atoms))
    if not models:
        if args['macro_ends'] or args['macro_close'] or args['xyz'] or args['traj']:
            permitted_models = ['*']
        else:
            permitted_models = [1]
    ...
    if not atoms:
        ...
        for part in tmp:
            if part == 'backbone':
                permitted_atoms = permitted_atoms + ["CA", "C", "N", "C3'", "C4'", "C5'", "O5'", "P", "O3'"]
            elif part == 'all':
                permitted_atoms = ['*']
                break
            elif part:
                permitted_atoms.append(part)
    permitted_atoms = list(set(permitted_atoms))
    if type(debug) is list: print(time.strftime("%d %b %H:%M:%S",
                                                time.gmtime()) + " Permitted atoms. The values supplied:\n" + color.BOLD + "Models:" + color.END + str(
        permitted_models) + color.BOLD + "\tChains:" + color.END + str(
        permitted_chains) + color.BOLD + "\tResidues:" + color.END + str(
        permitted_residues) + color.BOLD + "\tAtoms:" + color.END + str(permitted_atoms))
...
def read_coordinates(pdb_file, args):  # reading coordinates from PDB file
    debug = args['debug']
    permitted_models, permitted_chains, permitted_residues, permitted_atoms = parse_atoms(
        args)  # decide, which models/chains/residues/atoms to store
    with open(args['input_file'], 'r') as input_file:
        if type(debug) is list: print(time.strftime("%d %b %H:%M:%S", time.gmtime()) + " File " + args[
            'input_file'] + " opened. Reading the coordinates.")
        ...
        if current_model in permitted_models or '*' in permitted_models:
            ...
            store = True
            if type(debug) is list: print(
                        time.strftime("%d %b %H:%M:%S", time.gmtime()) + color.GREEN + " Found model " + str(
                    current_model) + color.END)
        ...
        for line in input_file:
            if line[0:6] == "MODEL ":
                ...
                if (current_model in permitted_models or '*' in permitted_models):
                    ...
                    if current_model > 1:
                        ...
                        current_chain = 1
                        if type(debug) is list: print(time.strftime("%d %b %H:%M:%S",
                                                                    time.gmtime()) + color.GREEN + " Found model " + str(
                            current_model) + color.END)
                else:
                    store = False
                model_created = True
            ...
            if line[0:6] == "ENDMDL" and store:
                if not model_cleaned and store:
                    pdb_file.clean(current_model, args['altloc'], permitted_atoms,
                                   permitted_residues)  # cleaning the data
                    result += str(print_coordinates(pdb_file, current_model, args))
                    model_cleaned = True
                    model_created = False
            ...
            if line[0:6] == "ATOM  ":
                if not model_created:
                    ...
                    if current_model in permitted_models or '*' in permitted_models:
                        ...
                        current_chain = 1
                        store = True
                        if type(debug) is list and current_model > 1: print(time.strftime("%d %b %H:%M:%S",
                                                                                          time.gmtime()) + color.GREEN + " Found model " + str(
                            current_model) + color.END)
                    else:
                        store = False
                ...
                if store: pdb_file.models['mod' + str(current_model)].add_atom(line, current_chain, permitted_chains,
                                                                               permitted_residues, permitted_atoms)
    ...
    if not model_cleaned and store:
        ...
        model_cleaned = True
    if type(debug) is list: print(time.strftime("%d %b %H:%M:%S", time.gmtime()) + " File " + args[
        'input_file'] + " done with the coordinates.")
...
def print_coordinates(pdb_file, model, args):  # printing coordinates to xyz files
    ...
    if type(debug) is list and ('models' in debug or 'chains' in debug or 'residues' in debug or 'atoms' in debug):
        pdb_file.models['mod' + str(model)].print_model_info(debug)
    if output == 'files':
        pdb_file.models['mod' + str(model)].print_coordinates(pdb_file, args)
        return
    result_pdb = pdb_file.models['mod' + str(model)].print_pdb()[:-1]
    result_xyz = pdb_file.models['mod' + str(model)].print_xyz(args['fourcolumn'], args['traj'], args['columns'],
                                                               args['hetatoms'])
    if output == 'screen':
...
def macrolink_ends_distance(perm, orient, distance_matrix):
    ...
    for k in range(len(perm) - 1):
        distance += distance_matrix[
            str(perm[k]) + str(orient[k]) + '-' + str(perm[k + 1]) + macrolink_rev_termini(str(orient[k]))]
    distance += distance_matrix[
        str(perm[-1]) + str(orient[-1]) + '-' + str(perm[0]) + macrolink_rev_termini(str(orient[0]))]
    return distance


def macrolink_ends_check_distances(loop_arrangement, chains, distance_matrix):
    for k in range(len(loop_arrangement[0]) - 1):
        d = distance_matrix[str(loop_arrangement[0][k]) + str(loop_arrangement[1][k]) + '-' + str(
            loop_arrangement[0][k + 1]) + macrolink_rev_termini(str(loop_arrangement[1][k]))]
        if d > 10:
            print(
                        'WARNING!!! It is possible that given chains do not form a macrocomponent. The distance between connected termini is ' + str(
                    d) + ' between ' + chains[k] + ' and ' + chains[k + 1] + ' for the best loop!')
    d = distance_matrix[str(loop_arrangement[0][-1]) + str(loop_arrangement[1][-1]) + '-' + str(
        loop_arrangement[0][0]) + macrolink_rev_termini(str(loop_arrangement[1][0]))]
    if d > 10:
        print(
                    'WARNING!!! It is possible that given chains do not form a macrocomponent. The distance between connected termini is ' + str(
                d) + ' between ' + chains[-1] + ' and ' + chains[0] + ' for the best loop!')
    return
...
def make_macrolink_ends_presentation(loop_arrangements, termini):
    ...
    for k in range(len(loop_arrangements[0])):
        ...
        presentation += termini[2 * loop_arrangements[0][k] + shift] + '-c-' + termini[
            2 * loop_arrangements[0][k] + (shift + 1) % 2] + '-b-'
...
def connected_print_xyz(coordinates, atom_list, debug, output='files'):
    ...
    for k in range(len(atom_list)):
        ...
        if len(coordinates[atom_list[k]]) != 3 and atom_list[k].split('_')[-1] != 'closure':
            result += (str(coordinates[atom_list[k]][3]) + ' ' + ' '.join(
                coordinates[atom_list[k]][:3]) + ' ' + ' '.join(coordinates[atom_list[k]][4:])).strip() + '\n'
...
def connected_print_pdb(coordinates, atom_list, debug,
                        output='files'):  # create completelly artificial PDB file, just to watch it in PyMol/Chimera/VMD/etc
    result = ''
    tot = 1
    for k in range(len(atom_list)):
        if atom_list[k].split('_')[-1] == 'closure':
        ...
        if k == 0:
            ...
            for l in range(n):
                ...
                result += "ATOM  %(atom)5s  CA  %(res_name)3s X%(res_nr)4s    %(x)8s%(y)8s%(z)8s  1.00  1.00          C   \n" % {
                    "atom": l + tot, "res_name": res_name, "res_nr": l + tot,
                    "x": '{0:.3f}'.format(float(last[0]) + l * delta[0]),
                    "y": '{0:.3f}'.format(float(last[1]) + l * delta[1]),
                    "z": '{0:.3f}'.format(float(last[2]) + l * delta[2])}
            tot += n
            last = coordinates[atom_list[k]][:3]
    result += "ATOM  %(atom)5s  CA  %(res_name)3s X%(res_nr)4s    %(x)8s%(y)8s%(z)8s  1.00  1.00          C   \n" % {
...
def build_macro_close(pairs, bridges, iteration):
    ...
    for k in range(len(pairs)):
        ...
        arrangement.append([str(pairs[k][0]) + '-' + str(bridges[pair_name][shift][0]),
                            str(pairs[k][1]) + '-' + str(bridges[pair_name][shift][1])])
...
def print_macrolink(pdb_file, args):  # printing components chosen automatically from a given set of chains to files
    ...
    while k < len(tmp):
        ...
        k += 1
    for x in tmp:
        if not (x.split('_')[0].isdigit() and len(x.split('_')) == 2 and len(x.split('_')[1]) == 1 and x.split('_')[
            1].isalpha()):  # check input
    ...
    if args['macro_ends']:
        ...
        for chain in tmp:
            termini.append(
                chain + '_' + str(pdb_file.models['mod' + chain.split('_')[0]].chains[chain.split('_')[1]].CEnd_number))
            termini.append(
                chain + '_' + str(pdb_file.models['mod' + chain.split('_')[0]].chains[chain.split('_')[1]].NEnd_number))
        for k in range(len(termini)):
            for l in range(2 * int(k / 2)):  # to avoid closing begin and end of the same chain
                ...
                if k % 2 == 0: c1 = 'C'
                if l % 2 == 0: c2 = 'C'
                distance_matrix[str(n1) + c1 + '-' + str(n2) + c2] = dist(coordinates[termini[k]][:3],
                                                                          coordinates[termini[l]][:3])
                distance_matrix[str(n2) + c2 + '-' + str(n1) + c1] = dist(coordinates[termini[k]][:3],
                                                                          coordinates[termini[l]][:3])
        ...
        for perm in permutations:
            for orient in orientations: test_arrangements.append(
                [perm, orient, macrolink_ends_distance(perm, orient, distance_matrix)])
    ...
    if args['macro_close']:
        bridges = {}
        len_vector = [float(pdb_file.models['mod' + chain.split('_')[0]].chains[chain.split('_')[1]].CEnd_number -
                            pdb_file.models['mod' + chain.split('_')[0]].chains[chain.split('_')[1]].NEnd_number) / 10
                      for chain in
                      chains.split(',')]  # how many residues from each chain we want to include (in this case it is 10)
        ...
        if not test_arrangements or test_arrangements[0] == [[], 999999]:
            print(
                "ERROR!!! It seems that there is no clear way to connect the chain or the chains supplied are to far away oto form a component. If you desire to connect these chains, please supply the list of inter-chain connections. I cannot guess them for you...")
            return
...
def print_commands(PDB, args):  # printing commands for Wanda's program
    ...
    for bridge in PDB.bridge_list:
        if bridge not in PDB.cross_bridge_list and PDB.bridges[bridge].peptide and PDB.bridges[
            bridge].size >= min_loop_length:
            if 'bridge_type' in args['commands']:
                result += PDB.bridges[bridge].type + ' '
            if 'ready' in args['commands']:
                result += './' + WandaLassoProgram + ' '
            result += args['input_file'] + '_' + PDB.bridges[bridge].res1_chain + '.xyz ' + str(
                PDB.bridges[bridge].res1_index) + ' ' + str(PDB.bridges[bridge].res2_index) + '\n'
    result = result[:-1]
...
def print_cross_bridges(PDB, args):  # printing cross-chain bridges to screen to screen
    if type(args['debug']) is list: print(time.strftime("%d %b %H:%M:%S",
                                                        time.gmtime()) + color.CYAN + " Printing cross-chains bridges" + color.END)
    result = ''
    for bridge in PDB.bridges:
        if bridge in PDB.cross_bridge_list:
            ...
            result += PDB.bridges[bridge].res1_chain + ' ' + str(PDB.bridges[bridge].res1_index) + ' ' + PDB.bridges[
                bridge].res2_chain + ' ' + str(PDB.bridges[bridge].res2_index) + '\n'
    result = result[:-1]
    if args['output'] == 'pipe':
        return result
    print(result)
...
def generate_closure(res1, res2, closure_type, com=False):
    ...
    if closure_type == 'direct': return ' '.join(
        [str(round((float(a) + float(b)) / 2, 3)) for a, b in zip(res1[1:4], res2[1:4])])
    if closure_type == 'one_point': return ' '.join(
        [str(float(a) + float(b)) for a, b in zip(com, generate_sphere_point(50))])
    if closure_type == 'two_points': return ' '.join(
        [str(float(a) + float(b)) for a, b in zip(com, generate_sphere_point(50))]) + '\n' + ' '.join(
        [str(float(a) + float(b)) for a, b in zip(com, generate_sphere_point(25))])
    if closure_type == 'parallel':
        ...
        return ' '.join([str(float(a) + float(b)) for a, b in zip(com, point1)]) + '\n' + ' '.join(
            [str(float(a) + float(b)) for a, b in zip(com, point1)])
    return '0 0 0'
...
def print_extended_bridges(pdb_file, args):
    ...
    for bridge in pdb_file.bridges:
        if args['noions'] and pdb_file.bridges[bridge].type != 'SS':
            continue
        chain1 = pdb_file.bridges[bridge].res1_chain
        chain2 = pdb_file.bridges[bridge].res2_chain
        if args['chains'] != '' and chain1 not in args['chains']:
            continue
        if args['chains'] != '' and chain2 not in args['chains']:
            continue
        res1 = pdb_file.bridges[bridge].res1_index
        res2 = pdb_file.bridges[bridge].res2_index
        if pdb_file.bridges[bridge].res1_type == 'HOH' or pdb_file.bridges[bridge].res2_type == 'HOH':
            continue
        ...
        if chain1 == chain2 and abs(res2 - res1) == 1:
            continue
    ...
    if chains:
        ...
        for bridge in bridges:
            if bridge[0] + '_' + bridge[1] not in coordinates and 'pipe' not in output:
                print("ERROR!!! It seems that there is no residue " + bridges_residues[
                    bridge[0] + '_' + bridge[1]] + '-' + bridge[0] + bridge[1] + ". Please fix this.")
                sys.exit()
            if bridge[2] + '_' + bridge[3] not in coordinates and 'pipe' not in output:
                print("ERROR!!! It seems that there is no residue " + bridges_residues[
                    bridge[2] + '_' + bridge[3]] + '-' + bridge[2] + bridge[3] + ". Please fix this.")
                sys.exit()
        if args['closure']:  # adding 'bridge' via closure. What type of closure will be decided later.
            for chain in chains:
                res1 = pdb_file.find_residue(0, chain.split('_')[1],
                                             pdb_file.find_chain(0, chain.split('_')[1]).NEnd_number)
                res2 = pdb_file.find_residue(0, chain.split('_')[1],
                                             pdb_file.find_chain(0, chain.split('_')[1]).CEnd_number)
    ...
    for k in range(len(extended_loops)):
        ...
        if 'pipe' not in output: print(
                    str(k) + '\t' + code + '.pdb_' + chh + '_' + str(k) + '.xyz' + '\t' + shortcut[k])
        if output == 'screen':
            if not args['opdb']:
                print(result_xyz[k])
            if not args['opdb'] and not args['oxyz']:
        ...
        if output == 'files':
            if type(args['debug']) is list: print(time.strftime("%d %b %H:%M:%S",
                                                                time.gmtime()) + ' Saving extended loops to files "' + code + '.pdb_' + chh + '_' + str(
                k) + '.xyz". Cycles in shorted version are (<-> denotes bridge):')
            if not args['opdb']:
                if 'closure' in shortcut[k]:
                    save_closure_xyz(code + '.pdb_' + chh + '_' + str(k) + '.xyz', result_xyz[k], args)
                else:
                    with open(code + '.pdb_' + chh + '_' + str(k) + '.xyz', 'w') as output_file:
                        output_file.write(result_xyz[k])
            if not args['opdb']:
                if 'closure' in shortcut[k]:
                    save_closure_pdb(code + '.pdb_' + chh + '_' + str(k) + '.pdb', result_pdb[k], args)
                else:
...
def suggest_bridges(PDB, args):  # printing close deterministic loops for further link analysis to screen
    if type(args['debug']) is list: print(
                time.strftime("%d %b %H:%M:%S", time.gmtime()) + color.CYAN + " Suggesting bridges" + color.END)
    bridges = []
    possible = []
    for bridge in PDB.bridges:
        if bridge not in PDB.cross_bridge_list and PDB.bridges[bridge].type != 'OTHER': bridges.append(bridge)
    for perm in itertools.combinations(list(range(len(bridges))), 2): possible.append(
        [bridges[perm[0]], bridges[perm[1]]])
    for perm in itertools.combinations(list(range(len(bridges))), 3): possible.append(
        [bridges[perm[0]], bridges[perm[1]], bridges[perm[2]]])
    for perm in itertools.combinations(list(range(len(bridges))), 4): possible.append(
        [bridges[perm[0]], bridges[perm[1]], bridges[perm[2]], bridges[perm[3]]])
    ...
    for bridge in possible:
        ...
        for k in order:
            bridgeClass = PDB.bridges[bridge[k]]
            result += name + bridgeClass.res1_chain + ' ' + str(bridgeClass.res1_index) + ' ' + str(
                bridgeClass.res2_index) + '#'
        result = result[:-1] + '\n'
    if args['output'] == 'pipe':
        return result[:-1]
    print(result[:-1])
...
def close_chains(pdb_file, k, j, args):
    ...
    return bool(float(args['factor'][0]) * (chain1.R + chain2.R) > sqrt(
        sum([(x - y) ** 2 for x, y in zip(chain1.com, chain2.com)])))


def suggest_chains(PDB, args):  # printing close chains for further link analysis to screen
    if type(args['debug']) is list: print(
                time.strftime("%d %b %H:%M:%S", time.gmtime()) + color.CYAN + " Suggesting chains" + color.END)
    chains = []
    ...
    while k < len(chains) and len(chains[k]) < 3:
        l = k + 1
        while l < len(chains):
            if len(chains[k]) == 2 and len(chains[l]) == 2 and len(
                list(set(chains[k] + chains[l]))) == 3: chains.append(list(set(chains[k] + chains[l])))
            if sorted([len(chains[k]), len(chains[l])]) == [2, 3] and len(
                list(set(chains[k] + chains[l]))) == 4: chains.append(list(set(chains[k] + chains[l])))
            l += 1
        k += 1
...
def find_cycles(pdb_file, bridges, coordinates, args, clean):
    ...
    for bridge in bridges:  # create bridge part of the graph and check if residues exist
        ...
        if bridge[2] + '_' + bridge[3] not in coordinates and bridge[3] != 'closure':
            ...
            sys.exit()
        if bridge[0] + '_' + bridge[1] not in graph:
            graph[bridge[0] + '_' + bridge[1]] = [['b', bridge[2] + '_' + bridge[3]]]
        else:
            graph[bridge[0] + '_' + bridge[1]].append(['b', bridge[2] + '_' + bridge[3]])
        if bridge[2] + '_' + bridge[3] not in graph:
            graph[bridge[2] + '_' + bridge[3]] = [['b', bridge[0] + '_' + bridge[1]]]
        else:
            graph[bridge[2] + '_' + bridge[3]].append(['b', bridge[0] + '_' + bridge[1]])
        if bridge[1] != 'closure' and bridge[3] != 'closure':
            d = dist(coordinates[bridge[0] + '_' + bridge[1]][:3], coordinates[bridge[2] + '_' + bridge[3]][:3])
            if d > 10: print("WARNING!!! The bridge " + bridge[0] + bridge[1] + ' <-> ' + bridge[2] + bridge[
                3] + ' seems to be wrong as its length is equal ' + str(d) + '.')
    ...
    for key in chain_residues:  # add the chain part of the graph (residues connected by the backbone)
        ...
        for k in range(len(residue_list) - 1):
            ...
            if residue_list[k] >= Nterm and residue_list[k + 1] >= Nterm and residue_list[k] <= Cterm and residue_list[
                k + 1] <= Cterm:
    ...
    if type(args['debug']) is list and (
            'merge' in args['debug'] or 'extended' in args['debug'] or 'macro' in args['debug']):
    ...
    if type(args['debug']) is list: print(time.strftime("%d %b %H:%M:%S", time.gmtime()) + ' Found ' + str(
        len(cycles)) + ' cycles. Cleaning cycles.')
    cycles = clean_cycles(cycles, bridges, clean)
    if type(args['debug']) is list: print(
                time.strftime("%d %b %H:%M:%S", time.gmtime()) + ' Cycles cleaned. Found ' + str(
            len(cycles)) + ' clean cycles.')
...
def merge_xyz_component(pdb_file, current_component, coordinates, bridges, output, args):
    if type(args['debug']) is list:
        print(time.strftime("%d %b %H:%M:%S", time.gmtime()) + ' Component ' + str(
            current_component) + '. I found the following bridges:')
    ...
    if len(cycles) > 1:
        print('ERROR!!! The bridges given in component ' + str(current_component) + ' form ' + str(
            len(cycles)) + ' valid components! Please specialize!')
        sys.exit()
    if len(cycles) == 0:
        print('ERROR!!! The bridges given in component ' + str(
            current_component) + ' cannot form a component! Please fix them!')
        sys.exit()
    ...
    if output == 'files':
        if type(args['debug']) is list: print(
                    time.strftime("%d %b %H:%M:%S", time.gmtime()) + ' Saving cycles to files "component_' + str(
                current_component) + '.xyz". Cycles in shorted version are (<-> denotes bridge):')
        if not args['opdb']:
...
def merge_xyz(pdb_file, args):  # printing components for macrolink to files
    ...
    debug = args['debug']
    if type(debug) is list: print(time.strftime("%d %b %H:%M:%S",
                                                time.gmtime()) + color.CYAN + " Preparing macrocomponents with the connections supplied by the user" + color.END)
    if not os.path.isfile(args['xyz'][0]):
        print(time.strftime("%d %b %H:%M:%S", time.gmtime()) + " No " + args['xyz'][
            0] + " file. Cannot do merging of the chain for macrocomponent analysis.")
        return
        ...
        for line in f:
            if line[0] == '#':
                ...
                if '_' in line.split()[0] and '_' not in line.split()[2]:
                    bridges[current_component].append(
                        [line.split()[0], line.split()[1], '1_' + line.split()[2], line.split()[3]])
                    chains[current_component].append(line.split()[0])
                    chains[current_component].append('1_' + line.split()[2])
                if '_' not in line.split()[0] and '_' in line.split()[2]:
                    bridges[current_component].append(
                        ['1_' + line.split()[0], line.split()[1], line.split()[2], line.split()[3]])
                    chains[current_component].append('1_' + line.split()[0])
                    chains[current_component].append(line.split()[2])
                if '_' not in line.split()[0] and '_' not in line.split()[2]:
                    bridges[current_component].append(
                        ['1_' + line.split()[0], line.split()[1], '1_' + line.split()[2], line.split()[3]])
                    chains[current_component].append('1_' + line.split()[0])
                    chains[current_component].append('1_' + line.split()[2])
    chains_total = []
...
class PDB_File:
    ...
    def add_compound(self, line):
        ...
        if line[10:80].lstrip()[0:8] == "MOLECULE": self.compounds[self.compound_list[-1]].add_molecule(
            line[10:80].lstrip())
        if line[10:80].lstrip()[0:5] == "CHAIN": self.compounds[self.compound_list[-1]].add_chains(line[10:80].lstrip())
        if line[10:80].lstrip()[0:8] == "FRAGMENT": self.compounds[self.compound_list[-1]].add_fragment(
            line[10:80].lstrip())
        if line[10:80].lstrip()[0:7] == "SYNONYM": self.compounds[self.compound_list[-1]].add_synonym(
            line[10:80].lstrip())
        if line[10:80].lstrip()[0:2] == "EC": self.compounds[self.compound_list[-1]].add_EC(line[10:80].lstrip())
        if line[10:80].lstrip()[0:10] == "ENGINEERED": self.compounds[self.compound_list[-1]].add_engineered(
            line[10:80].lstrip())
        if line[10:80].lstrip()[0:8] == "MUTATION": self.compounds[self.compound_list[-1]].add_mutation(
            line[10:80].lstrip())
        if line[10:80].lstrip()[0:13] == "OTHER_DETAILS": self.compounds[self.compound_list[-1]].add_details(
            line[10:80].lstrip())

    def add_source(self, line):
        if line[10:80].lstrip()[0:6] == "MOL_ID": self.current_compound = int(line.split()[-1].rstrip(';'))
        if line[10:80].lstrip()[0:10] == "SYNTHETIC": self.compounds['comp' + str(self.current_compound)].add_synthetic(
            line[10:80].lstrip())
        if line[10:80].lstrip()[0:19] == "ORGANISM_SCIENTIFIC": self.compounds[
            'comp' + str(self.current_compound)].add_org_sc(line[10:80].lstrip())
        if line[10:80].lstrip()[0:15] == "ORGANISM_COMMON": self.compounds[
            'comp' + str(self.current_compound)].add_org_com(line[10:80].lstrip())
        if line[10:80].lstrip()[0:14] == "ORGANISM_TAXID": self.compounds[
            'comp' + str(self.current_compound)].add_taxid(line[10:80].lstrip())  # add missing items from source
    ...
    def find_missing_residue(self, model, chain, residue_name):
        ...
        if residue_name - residue_shift < len(g_chain.residue_list) and g_chain.residue_list[
            residue_name - residue_shift - 1] == chain.residues[chain.residue_list_sorted[-1]].type: return \
        g_chain.residue_list[residue_name - residue_shift]
        # if the residue is in residue list (seqres) but the numbers in seqres are shifted
        try:
            current_residue_list = [chain.residues[chain.residue_list_sorted[k]].type for k in
                                    range(residue_name - residue_shift)]
    ...
    def remove_duplicated_bridges(self):
        ...
        while k < len(self.bridge_list):
            ...
            while l < len(self.bridge_list):
                ...
                if set([bridge1.res1_index, bridge1.res2_index]) == set(
                        [bridge2.res1_index, bridge2.res2_index]) and set(
                        [bridge1.res1_chain, bridge1.res2_chain]) == set([bridge2.res1_chain, bridge2.res2_chain]):
    ...
    def informations(self, debug):
        if type(debug) is list and (
                'pdb' in debug or 'models' in debug or 'chains' in debug or 'residues' in debug or 'atoms' in debug or 'bridges' in debug):
            ...
            if type(debug) is list and (
                    'models' in debug or 'chains' in debug or 'residues' in debug or 'atoms' in debug):
...
class Model:
    ...
    #### Adding part
    def add_atom(self, line, current_chain, chains, residues,
                 atoms):  # we create every chain, however only permitted chains are filled with residues (to save space)
        chain_name = line[21]
    ...
    def print_coordinates(self, pdb_file, args):
        if not args['ochains']:
            with open(args['input_file'] + '_extracted.pdb', 'a') as output_file:
                if args['input_file'] + '_extracted.pdb' not in pdb_file.created_PDB: pdb_file.created_PDB.append(
                    args['input_file'] + '_extracted.pdb')
            ...
            for chain in self.chains:
                if not args['oxyz']:
                    ...
                    with open(args['input_file'] + '_' + self.chains[chain].name + '.pdb', 'a') as output_file:
                        if args['input_file'] + '_' + self.chains[
                            chain].name + '.pdb' not in pdb_file.created_PDB: pdb_file.created_PDB.append(
                            args['input_file'] + '_' + self.chains[chain].name + '.pdb')
                ...
                if not args['opdb']:
                    with open(args['input_file'] + '_' + self.chains[chain].name + '.xyz', 'a') as output_file:
                        ...
                        output_file.write(
                            self.chains[chain].print_xyz(args['fourcolumn'], args['columns'], args['hetatoms']))
...
class Chain:
    ...
    def extract_nonpeptide(self):
        for k in range(len(self.residue_list)):
            if not self.residues[self.residue_list[k]].peptide: self.residue_nonpeptide_list.append(
                self.residue_list[k])
        ...
        while k < len(self.residue_list):  # for each residue found in file
            while len(self.residue_list_sorted) == 0:  # finding the first acceptable residue
                if len(self.residues[self.residue_list[k]].atom_list):
                    if not self.residues[self.residue_list[k]].hetatom:
                        self.residue_list_sorted.append(self.residue_list[k])  # "atom" residue with CA? Add it.
                    elif self.residues[self.residue_list[
                        k]].type in nucleotides + amino_acids:  # "hetatom" residue, but Amino Acid or nucleotide?
                        self.residue_list_sorted.append(
                            self.residue_list[k])  # add it and make it "peptide" = main chain
                        ...
                        for l in range(
                                len(pdb_file.bridge_list)):  # for example MSE. These should be connected via "LINK" line
                            ...
                            if (chain1, chain2) == (self.name, self.name) and set((res1, res2)) == set(
                                    (index1, index2)) and pdb_file.bridges[
                                pdb_file.bridge_list[l]].type != 'OTHER':  # we accept all bridges but "OTHER"
                                self.residue_list_sorted.append(
                                    self.residue_list[k])  # if the bridge connect the residue and the next one,
            ...
            for l in range(k, len(self.residue_list)):
                if len(self.residues[self.residue_list[
                    l]].atom_list): break  # in case there is no main atom in res. In such case the resiude is doubled, TODO
            k = l
            if not len(self.residues[self.residue_list[k]].atom_list): break  # last residues do not have the main atoms
            n = self.residues[self.residue_list[k]].name - self.residues[
                self.residue_list_sorted[-1]].name  # n - sequential distance between residues
            if n == 1 and not self.residues[self.residue_list_sorted[-1]].hetatom and not self.residues[
                self.residue_list[k]].hetatom:  # n == 1 - no gap.
                d = self.distance(self.residues[self.residue_list_sorted[-1]], self.residues[self.residue_list[k]])
                if (d < 2.0): self.warning += 'WARNING!!! Unnatural distance ' + str(
                    d) + ' (< 2.0 A) between residues ' + str(
                    self.residues[self.residue_list_sorted[-1]].name) + ' and ' + str(
                    self.residues[self.residue_list[k]].name) + '.\n'
                if (d > 4.2): self.warning += 'WARNING!!! Unnatural distance ' + str(
                    d) + ' (> 4.2 A) between residues ' + str(
                    self.residues[self.residue_list_sorted[-1]].name) + ' and ' + str(
                    self.residues[self.residue_list[k]].name) + '.\n'
            ...
            if n > 1 and not self.residues[self.residue_list_sorted[-1]].hetatom and not self.residues[
                self.residue_list[k]].hetatom:  # gap between residues
                if n >= artifact_gap_size: self.warning += 'WARNING!!! In chain ' + self.name + ' there is a gap of length ' + str(
                    n) + ' between residues ' + str(self.residues[self.residue_list_sorted[-1]].name) + ' and ' + str(
                    self.residues[self.residue_list[k]].name) + '.\n'
                ...
                for j in range(1, n):
                    ...
                    line = "ATOM  %(atom)5s  CA  %(resname)3s %(chain)1s%(res_nr)4s    %(x)8s%(y)8s%(z)8s  1.00  1.00           C  \n" % {
                        ...
                        "x": '{0:.3f}'.format(float(coordinates1[0]) + j * delta[0], 3),
                        "y": '{0:.3f}'.format(float(coordinates1[1]) + j * delta[1], 3),
                ...
                self.residues[self.residue_list_sorted[-1]].NEnd = False
            if n == 1 and (
                    self.residues[self.residue_list_sorted[-1]].hetatom or self.residues[self.residue_list[k]].hetatom):
                bridge_exists = False
                for l in range(len(pdb_file.bridge_list)):
                    ...
                    if (chain1, chain2) == (self.name, self.name) and set((res1, res2)) == set((index1, index2)) and \
                            pdb_file.bridges[pdb_file.bridge_list[l]].type != 'OTHER':
                        bridge_exists = True
                        break
                if bridge_exists:
                    d = self.distance(self.residues[self.residue_list_sorted[-1]], self.residues[self.residue_list[k]])
                    ...
                    self.residues[self.residue_list_sorted[-1]].NEnd = False
                    self.residues[self.residue_list_sorted[-1]].peptide = True
            if n > 1 and (self.residues[self.residue_list_sorted[-1]].hetatom and not self.residues[
                self.residue_list[k]].hetatom):
                bridge_exists = False
                for l in range(len(pdb_file.bridge_list)):
                    chain1 = pdb_file.bridges[pdb_file.bridge_list[l]].res1_chain
                    chain2 = pdb_file.bridges[pdb_file.bridge_list[l]].res2_chain
    ...
    def print_chain_info(self, debug):
        ...
        print(color.BOLD + "\t\tNEnd:\t\t" + color.END + self.residues[self.residue_list[self.NEnd]].type + '_' + str(
            self.residues[self.residue_list[self.NEnd]].name))
        print(color.BOLD + "\t\tCEnd:\t\t" + color.END + self.residues[self.residue_list[self.CEnd]].type + '_' + str(
            self.residues[self.residue_list[self.CEnd]].name))
        ...
        result = '\t\t['
        for k in range(len(self.residue_list_sorted)):
            if self.residues[self.residue_list[k]].missing:
                result += color.RED + str(self.residues[self.residue_list[k]].name) + color.END + ', '
            elif not self.residues[self.residue_list[k]].peptide:
                result += color.YELLOW + str(self.residues[self.residue_list[k]].name) + color.END + ', '
            elif self.residues[self.residue_list[k]].hetatom:
                result += color.PURPLE + str(self.residues[self.residue_list[k]].name) + color.END + ', '
            else:
                result += str(self.residues[self.residue_list[k]].name) + ', '
        result = result[:-2]
        result += ']'
        print(result)
        print(
                    color.BOLD + "\t\tColor code:\t" + color.END + "Peptide\t\t" + color.RED + "Missing\t\t" + color.END + color.YELLOW + "Nonpeptide\t\t" + color.END + color.PURPLE + "Peptide hetatom" + color.END)
        print("\t\t________________________________\n")
...
class General_chain:
    ...
    def print_general_chain_info(self, debug):
        ...
        if type(debug) is list and ('residues' in debug or 'atoms' in debug): print(
                    "\t\t\t" + str(self.residue_list_missing))
        print(color.BOLD + "\t\tZero occ res:\t" + color.END + str(len(self.zero_occupancy_residues)))
        if type(debug) is list and ('residues' in debug or 'atoms' in debug): print(
                    "\t\t\t" + str(self.zero_occupancy_residues))
...
class Residue:
    def __init__(self, line, chain, model):
        ...
        if self.type in amino_acids + nucleotides:
            self.standard = True
        else:
            self.standard = False
        self.atoms = {}
    ...
    def find_main(self, altloc):  # get the index of main atom in the list of atoms
        ...
        for k in range(len(self.atom_list)):
            if self.atoms[self.atom_list[k]].name.strip() == 'CA' and self.atoms[
                self.atom_list[k]].altloc.strip() == altloc:
                self.main = k
                break
            if self.atoms[self.atom_list[k]].name.strip() == 'CA' and self.atoms[
                self.atom_list[k]].altloc.strip() == '':
                self.main = k
                break
            if self.atoms[self.atom_list[k]].name.strip() == 'CA':
                self.main = k
                break
            if self.atoms[self.atom_list[k]].name.strip() == "C3'" and self.atoms[
                self.atom_list[k]].altloc.strip() == altloc:
                self.main = k
                break
            if self.atoms[self.atom_list[k]].name.strip() == "C3'" and self.atoms[
    ...
    def clean_atom_list(self, altloc):
        for atom in self.atom_list:
            if self.atoms[atom].altloc.strip() == '' or self.atoms[atom].altloc.strip() == altloc or self.atoms[
                atom].main:
    ...
    def print_residue_info(self, debug):
        ...
        if self.standard: print(
                    color.BOLD + "\t\t\tStandard:\t" + color.END + color.GREEN + str(self.standard) + color.END)
        if not self.standard: print(
                    color.BOLD + "\t\t\tStandard:\t" + color.END + color.RED + str(self.standard) + color.END)
        print(color.BOLD + "\t\t\tNum of atoms:\t" + color.END + str(len(self.atom_list)))
        if not self.missing: print(
                    color.BOLD + "\t\t\tMissing:\t" + color.END + color.GREEN + str(self.missing) + color.END)
        if self.missing: print(color.BOLD + "\t\t\tMissing:\t" + color.END + color.RED + str(self.missing) + color.END)
        if not self.hetatom: print(
                    color.BOLD + "\t\t\tHeteroatom:\t" + color.END + color.GREEN + str(self.hetatom) + color.END)
        if self.hetatom: print(
                    color.BOLD + "\t\t\tHeteroatom:\t" + color.END + color.RED + str(self.hetatom) + color.END)
        if not self.peptide: print(
                    color.BOLD + "\t\t\tPeptide:\t" + color.END + color.RED + str(self.peptide) + color.END)
        if self.peptide: print(
                    color.BOLD + "\t\t\tPeptide:\t" + color.END + color.GREEN + str(self.peptide) + color.END)
...
class Atom:
    ...
    def print_xyz(self, res_index, four, column_format):
        if column_format:
            ...
            return result
        if four:
            return str(res_index) + " " + str(self.coordinates[0]) + " " + str(self.coordinates[1]) + " " + str(
                self.coordinates[2]) + "\n"
        else:
            return str(res_index) + " " + str(self.coordinates[0]) + " " + str(self.coordinates[1]) + " " + str(
                self.coordinates[2]) + " " + self.resname + "\n"
...
class Bridge:
    ...
    def decide_peptide(self, pdb_file):
        self.peptide = pdb_file.find_residue(0, self.res1_chain, self.res1_index).peptide and pdb_file.find_residue(0,
                                                                                                                    self.res2_chain,
                                                                                                                    self.res2_index).peptide

    def decide_exists(self, pdb_file):
        try:
            res1 = pdb_file.find_residue(0, self.res1_chain, self.res1_index)
        except KeyError:
            self.exists = False
            return
        try:
            res2 = pdb_file.find_residue(0, self.res2_chain, self.res2_index)
        except KeyError:
            self.exists = False
            return
    ...
    def bond_type(self):
        if self.res1_atom.strip()[0] == 'S' and self.res2_atom.strip()[0] == 'S':
            self.type = 'SS'
        elif self.res1_atom.strip()[0] == 'C' and self.res2_atom.strip()[0] == 'N':
            if self.res1_type == 'GLU' and self.res1_atom.strip() == 'CD' and self.res2_type == 'LYS' and self.res2_atom.strip() == 'NZ':
                self.type = 'AMIDE'
            elif self.res1_type == 'ASP' and self.res1_atom.strip() == 'CG' and self.res2_type == 'LYS' and self.res2_atom.strip() == 'NZ':
                self.type = 'AMIDE'
            elif self.res1_CEnd and self.res2_type == 'LYS' and self.res2_atom.strip() == 'NZ':
                self.type = 'AMIDE'
            elif self.res1_type == 'GLU' and self.res1_atom.strip() == 'CD' and self.res2_NEnd:
                self.type = 'AMIDE'
            elif self.res1_type == 'ASP' and self.res1_atom.strip() == 'CG' and self.res2_NEnd:
                self.type = 'AMIDE'
            else:
                self.type = 'AMIDE-like'
        elif self.res1_atom.strip()[0] == 'N' and self.res2_atom.strip()[0] == 'C':
            if self.res1_type == 'LYS' and self.res1_atom.strip() == 'NZ' and self.res2_type == 'GLU' and self.res2_atom.strip() == 'CD':
                self.type = 'AMIDE'
            elif self.res1_type == 'LYS' and self.res1_atom.strip() == 'NZ' and self.res2_type == 'ASP' and self.res2_atom.strip() == 'CG':
                self.type = 'AMIDE'
            elif self.res1_NEnd and self.res2_type == 'GLU' and self.res2_atom.strip() == 'CD':
                self.type = 'AMIDE'
            elif self.res1_NEnd and self.res2_type == 'ASP' and self.res2_atom.strip() == 'CG':
                self.type = 'AMIDE'
            elif self.res1_type == 'LYS' and self.res1_atom.strip() == 'NZ' and self.res2_CEnd:
                self.type = 'AMIDE'
            else:
                self.type = 'AMIDE-like'
        elif self.res1_atom.strip()[0] == 'C' and self.res2_atom.strip()[0] == 'O':
            if self.res1_type == 'GLU' and self.res1_atom.strip() == 'CD' and self.res2_type == 'THR' and self.res2_atom.strip() == 'OG1':
                self.type = 'ESTER'
            elif self.res1_type == 'GLU' and self.res1_atom.strip() == 'CD' and self.res2_type == 'SER' and self.res2_atom.strip() == 'OG':
                self.type = 'ESTER'
            elif self.res1_type == 'ASP' and self.res1_atom.strip() == 'CG' and self.res2_type == 'THR' and self.res2_atom.strip() == 'OG1':
                self.type = 'ESTER'
            elif self.res1_type == 'ASP' and self.res1_atom.strip() == 'CG' and self.res2_type == 'SER' and self.res2_atom.strip() == 'OG':
                self.type = 'ESTER'
            elif self.res1_CEnd and self.res2_type == 'SER' and self.res2_atom.strip() == 'OG':
                self.type = 'ESTER'
            elif self.res1_CEnd and self.res2_type == 'THR' and self.res2_atom.strip() == 'OG1':
                self.type = 'ESTER'
            else:
                self.type = 'ESTER-like'
        elif self.res1_atom.strip()[0] == 'O' and self.res2_atom.strip()[0] == 'C':
            if self.res1_type == 'THR' and self.res1_atom.strip() == 'OG1' and self.res2_type == 'GLU' and self.res2_atom.strip() == 'CD':
                self.type = 'ESTER'
            elif self.res1_type == 'THR' and self.res1_atom.strip() == 'OG1' and self.res2_type == 'ASP' and self.res2_atom.strip() == 'CG':
                self.type = 'ESTER'
            elif self.res1_type == 'SER' and self.res1_atom.strip() == 'OG' and self.res2_type == 'GLU' and self.res2_atom.strip() == 'CD':
                self.type = 'ESTER'
            elif self.res1_type == 'SER' and self.res1_atom.strip() == 'OG' and self.res2_type == 'ASP' and self.res2_atom.strip() == 'CG':
                self.type = 'ESTER'
            elif self.res1_type == 'THR' and self.res1_atom.strip() == 'OG1' and self.res2_CEnd:
                self.type = 'ESTER'
            elif self.res1_type == 'SER' and self.res1_atom.strip() == 'OG' and self.res2_CEnd:
                self.type = 'ESTER'
            else:
                self.type = 'ESTER-like'
        elif self.res1_atom.strip()[0] == 'C' and self.res2_atom.strip()[0] == 'S':
            if self.res1_type == 'GLU' and self.res1_atom.strip() == 'CD' and self.res2_type == 'CYS' and self.res2_atom.strip() == 'SG':
                self.type = 'THIOESTER'
            elif self.res1_type == 'ASP' and self.res1_atom.strip() == 'CG' and self.res2_type == 'CYS' and self.res2_atom.strip() == 'SG':
                self.type = 'THIOESTER'
            elif self.res1_CEnd and self.res2_type == 'CYS' and self.res2_atom.strip() == 'SG':
                self.type = 'THIOESTER'
            elif self.res1_CEnd and self.res2_type == 'CYS' and self.res2_atom.strip() == 'SG':
                self.type = 'THIOESTER'
            else:
                self.type = 'THIOESTER-like'
        elif self.res1_atom.strip()[0] == 'S' and self.res2_atom.strip()[0] == 'C':
            if self.res1_type == 'CYS' and self.res1_atom.strip() == 'SG' and self.res2_type == 'GLU' and self.res2_atom.strip() == 'CD':
                self.type = 'THIOESTER'
            elif self.res1_type == 'CYS' and self.res1_atom.strip() == 'SG' and self.res2_type == 'ASP' and self.res2_atom.strip() == 'CD':
                self.type = 'THIOESTER'
            elif self.res1_type == 'CYS' and self.res1_atom.strip() == 'SG' and self.res2_CEnd:
                self.type = 'THIOESTER'
            elif self.res1_type == 'CYS' and self.res1_atom.strip() == 'SG' and self.res2_CEnd:
                self.type = 'THIOESTER'
            else:
    ...
    def print_bridge_info(self):
        print(color.BOLD + "\tRes1 index:\t" + color.END + str(
            self.res1_index) + color.BOLD + "\tRes2 index:\t" + color.END + str(self.res2_index))
        print(
                    color.BOLD + "\tRes1 type:\t" + color.END + self.res1_type + color.BOLD + "\tRes2 type:\t" + color.END + self.res2_type)
        print(
                    color.BOLD + "\tRes1 chain:\t" + color.END + self.res1_chain + color.BOLD + "\tRes2 chain:\t" + color.END + self.res2_chain)
        print(
                    color.BOLD + "\tRes1 ins:\t" + color.END + self.res1_insert + color.BOLD + "\tRes2 ins:\t" + color.END + self.res2_insert)
        print(
                    color.BOLD + "\tRes1 symmetry:\t" + color.END + self.res1_symop + color.BOLD + "\tRes2 symmetry:\t" + color.END + self.res2_symop)
        print(
                    color.BOLD + "\tRes1 atom:\t" + color.END + self.res1_atom.strip() + color.BOLD + "\tRes2 atom:\t" + color.END + self.res2_atom.strip())
        print(color.BOLD + "\tRes1 NEnd:\t" + color.END + str(
            self.res1_NEnd) + color.BOLD + "\tRes2 NEnd:\t" + color.END + str(self.res2_NEnd))
        print(color.BOLD + "\tRes1 CEnd:\t" + color.END + str(
            self.res1_CEnd) + color.BOLD + "\tRes2 CEnd:\t" + color.END + str(self.res2_CEnd))
        if self.dist > 4.2 or self.dist < 2.0: print(
                    color.BOLD + "\tDistance:\t" + color.END + color.RED + str(self.dist) + color.END)
        if self.dist <= 4.2 and self.dist >= 2.0: print(
                    color.BOLD + "\tDistance:\t" + color.END + color.GREEN + str(self.dist) + color.END)
        if self.type == 'SS':
            print(color.BOLD + "\tType:\t\t" + color.END + color.YELLOW + self.type + color.END)
        elif self.type == 'AMIDE' or self.type == 'AMIDE-like':
            print(color.BOLD + "\tType:\t\t" + color.END + color.RED + self.type + color.END)
        elif self.type == 'ESTER' or self.type == 'ESTER-like':
            print(color.BOLD + "\tType:\t\t" + color.END + color.GREEN + self.type + color.END)
        elif self.type == 'THIOESTER' or self.type == 'THIOESTER-like':
            print(color.BOLD + "\tType:\t\t" + color.END + color.BLUE + self.type + color.END)
        else:
            print(color.BOLD + "\tType:\t\t" + color.END + self.type)
        print(color.BOLD + "\tLoop size:\t" + color.END + str(self.size))
        if self.peptide:
            print(color.BOLD + "\tPeptide:\t" + color.END + color.GREEN + str(self.peptide) + color.END)
        else:
            print(color.BOLD + "\tPeptide atoms:\t" + color.END + color.RED + str(self.peptide) + color.END)
        if self.exists:
            print(color.BOLD + "\tExists:\t\t" + color.END + color.GREEN + str(self.exists) + color.END)
        else:
            print(color.BOLD + "\tAtoms Exist:\t\t" + color.END + color.RED + str(self.exists) + color.END)
        print("\t__________________________________")
...
#################################################################")
    ...
    parser.add_argument('-b', '--bridges', action="store_true", dest="bridges", default=False,
                        help="Write information about inter-chain bridges.")
    parser.add_argument('-e', '--extended', action="store_true", dest="extended", default=False,
                        help="Write information about extended-type loops (via ions, double bridges etc).")
    parser.add_argument('-s', '--suggest_bridges', action="store_true", dest="sbridge", default=False,
                        help="Suggest bridges for deterministic link analysis based on distance criterium.")
    parser.add_argument('-z', '--suggest_chains', action="store_true", dest="schain", default=False,
                        help="Suggest chains for probabilistic link analysis based on distance criterium.")
    parser.add_argument('-n', '--macro_ends', nargs='*', dest="macro_ends", default=False,
                        help="Prepare xyz files with merged chains. Chains are connected by the termini.")
    parser.add_argument('-m', '--macro_close', nargs='*', dest="macro_close", default=False,
                        help="Prepare xyz files with merged chains. Chains are connected by the closest residues.")
    parser.add_argument('-x', '--xyzfiles', nargs=1, dest="xyz", default=False,
                        help="Prepare xyz files with merged chains. The list of chain connections must be supplied.")
    ...
    parser.add_argument('--output', nargs=1, dest="output", default=['files'], help="Output of the coordinates.",
                        choices=['files', 'screen', 'pipe'])
    parser.add_argument('--columns', nargs='*', dest="columns", default='',
                        help="Format of columns for xyz files. Accepted descriptors are index, x, y, z, name, element, residue, chain, model, residue_index, bfactor, occupancy, charge, insertion, alt_location and '|' as separator. Other values will be omitted.")
    parser.add_argument('--only_general', '--general_only', action="store_true", dest="ogeneral", default=False,
                        help="Print only one file with extracted coordinates chosen.")
    parser.add_argument('--only_chains', '--chains_only', action="store_true", dest="ochains", default=False,
                        help="Print only files for each chain.")
    parser.add_argument('--only_pdb', '--pdb_only', action="store_true", dest="opdb", default=False,
                        help="Print only PDB files.")
    parser.add_argument('--only_xyz', '--xyz_only', action="store_true", dest="oxyz", default=False,
                        help="Print only XYZ files.")
    parser.add_argument('--hetatoms', action="store_true", dest="hetatoms", default=False, help="Print also hetatoms.")
    parser.add_argument('--sug_chain_factor', nargs=1, dest="factor", default=['0.85'],
                        help="Factor used in searching close chains used in link analysis.")
    parser.add_argument('--commands', nargs='*', dest="commands", default=['bridge_type'],
                        choices=['no', 'bridge_type', 'ready'], help="Commands format for Wanda lasso program.")
    parser.add_argument('--noions', action="store_true", dest="noions", default=False,
                        help="If not to include the ions.")
    parser.add_argument('--closure', nargs=1, dest="closure", default=[], help="To close the protein and how.",
                        choices=['one_point', 'two_points', 'parallel', 'direct'])
    parser.add_argument('--closure_num', nargs=1, dest="closure_num", default=100, type=int,
                        help="Number of closures to be done (for two_points, parallel, and one_point).")
    parser.add_argument('--bridge_len', nargs=1, dest="bridge_len", default=[15],
                        help="Maximal bridge length in Angstrems.")
    parser.add_argument('-l', '--altloc', nargs=1, dest="altloc", default=['A'],
                        help="Alternative location of atom to choose. Default 'A'.")
    parser.add_argument('-d', '--debug', nargs='*',
                        choices=['pdb', 'compounds', 'models', 'chains', 'residues', 'atoms', 'coords_pdb',
                                 'coords_xyz', 'bridges', 'extended', 'merge'], dest="debug", default=False,
    ...
    if not (args['macro_ends'] or args['macro_close'] or args['xyz'] or args['extended']):
        ...
        if not args['bridges'] and not args['sbridge'] and not args['schain']: print_commands(PDB,
                                                                                              args)  # printing commands to screen
    if args['macro_ends'] or args['macro_close']: print_macrolink(PDB,
                                                                  args)  # printing components chosen automatically from a given set of chains to files
    ...
    if args['sbridge']: suggest_bridges(PDB,
                                        args)  # printing close deterministic loops for further link analysis to screen
